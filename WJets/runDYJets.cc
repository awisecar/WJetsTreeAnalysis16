void runDYJets(int doWhat = 0, int doQCD = 0, int doSysRunning = 0)
{
    ...
    int doBJets      = 0; //no btag information used
    ...
    if (doWhat == 10 || doWhat == 100) {
        for (unsigned int i(0); i < NSystData; i++) {
            ...
            ZJetsAndDPS DMudata0(lepSelection+"_13TeV_Data_dR_5311_List_0", 1., 1., 1, doDataEff, dataSyst[i], dataDir[i], 1, jetPtMin, jetPtMax, ZPtMin, ZEtaMin, ZEtaMax, METcut, 0, jetEtaMin, jetEtaMax);
            DMudata0.Loop(1, 0, doQCD, doSSign, doInvMassCut, doBJets, doPUStudy, doFlat, doRoch, doVarWidth, hasPartonInfo);
