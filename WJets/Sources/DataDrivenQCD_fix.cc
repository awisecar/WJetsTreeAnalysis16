...
void FuncDataDrivenQCD(string variable, TFile *fData[], TFile *fMC[][NMC], TFile *fOut){
    ...
    // followed as described in AN2014_114_v13, pages 10-12
    ...
    if (variable.find("ZNGoodJets") == string::npos){
        ...
        // step 0 : initial normalization of Wjets to data (before calc. QCD)
        double NormFactor = (hData[0]->Integral() - hBack[0]->Integral() ) / hSignal[0]->Integral() ;
        cout << " Normalization factor for Wjets signal: " << NormFactor << " , Data:  " << hData[0]->Integral() << " , Wjets Signal MC:  " << hSignal[0]->Integral() << " , Total BG MC:   " <<  hBack[0]->Integral() << endl;
        /*
        // for a quick look at QCD contribution in each region
        for ( int i = 0  ; i < NQCD ; i++){
            cout << "initial integral/event count for QCD (Data - MC), Region Number " << i << " : " << hData[i]->Integral() - ( hBack[i]->Integral() + hSignal[i]->Integral() ) << "integral signal" << hSignal[i]->Integral() << endl;
        }
        */
        ...
        for ( int j = 0  ; j < 12 ; j++){
            ...
            cout << "!!!!!!!!!!!! NormFactor f_w for calculation number " << j << ":" << NormFactor << endl;
            cout << "Note: we don't calculate error for NormFactor f_w explicitly, but it ends up scaling the error on the histograms used to calculate quantities in regions B, C, and D" << endl;
            
            // step 1: calculate QCD in all 4 regions
            for ( int i = 0  ; i < NQCD ; i++){
            ...
            // step 2: calculate isolation fake rate from regions B and D
            ...
            cout << "!!!!!!!!!!!! Ratio of regions B to D:  " << NormFactorISO << " +- " << NormFactIsoError << " for calculation number" << i << ", QCD count in region B:   " << hQCD[1]->Integral() << ", QCD count in region D: " << hQCD[3]->Integral() <<  endl;
            
            // step 3 : isolation fake-rate from step 2 is applied to QCD[2] (region C) to get QCD[0] (region A, the signal region), applied from bins 1 to Nbins, so not applied to under- or overflow bins
        ...
        cout << " !!!!!!!!!!!!" << endl;
        cout << " Print out final NormFactor f_w used for QCD calculation: " << NormFactor_o[0] << endl;
        cout << " Print out final NormFactor ISO and its error for the variable: " << variable << endl;
        ...
        */
        ...
        // for ZNGoodJets dist.
        // andrew -- for both ZNGoodJets_Zexc and ZNGoodJets_Zinc?
        cout << "Print variable: " << variable << endl;
        for ( int i = 0  ; i < NQCD ; i++){
            scaledMC[i] = (TH1D*) hSignal[i]->Clone();
            hQCD[i] = (TH1D*) hData[i]->Clone();
        }
        
        //Note: Things are done bin-by-bin here for different jet multiplicities
        //For all other histograms we just integrate over the whole distribution to calculate the factors
        for (int m = 1; m <= hData[0]->GetNbinsX(); m++){
            cout << " -------- processing bin number : " << m << endl;
            // step 0 : initial normalization of Wjets and data
            // compute for first use
            double NormFactor(1);
            if(hSignal[0]->GetBinContent(m) > 0){NormFactor = (hData[0]->GetBinContent(m) - hBack[0]->GetBinContent(m)) / hSignal[0]->GetBinContent(m) ;}
            else {NormFactor = 1.;}
            
            for ( int j = 0  ; j < 12 ; j++){
                //recompute for updated hQCD[0]
                if ( j > 0 )  {
                ...
                cout << " !!!!!!!!!!!!" << endl;
                cout << " ratio of regions B to D: " << NormFactorISO << "for calculation number " << j <<  ", QCD count in region B:  " << hQCD[1]->GetBinContent(m) << ", QCD count in region D: " << hQCD[3]->GetBinContent(m) <<  endl;
        ...
        cout << "   print out error in each jet mult. bin for the variable: " << variable << endl;
        cout << " \t" << "# event QCD in SR" << " \t" << "error " << " \t" << "%error" << " \t" << "fw" << " \t" << "f_B/D" << " \t" << "%error f_B/D" << endl;
